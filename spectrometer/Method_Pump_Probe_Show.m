classdef Method_Pump_Probe < Method
    ...
    methods (Access = protected)
        ...
        function InitializeMainPlot(obj)
            ...
            %     end
            obj.hPlotMain(1) = plot(obj.hMainAxes,obj.freq,obj.result.data(1,:));
            %    obj.hPlotMain(2) = plot(obj.hMainAxes,obj.freq,obj.result.noise(1,:),'r');
            set(obj.hPlotMain(1),'XDataSource','obj.freq',...
                'YDataSource',['obj.result.data(' num2str(1) ',:)'])
            %    set(obj.hPlotMain(2),'XDataSource','obj.freq',...
            %          'YDataSource',['obj.result.noise(' num2str(1) ',:)'])
        ...
        function ProcessSampleSort(obj)
            ...
            switch obj.nChopStates
                case 1
                    %do nothing because
                case 2
                    out(1,:) = (obj.sample(obj.ind_chopper,:)<20000);
                    out(2,:) = ~out(1,:);
                otherwise
                    error('SRGLAB:unknowncase','nSignals = %f not allowed. only 1, 2, 4 supported',nChopStates)
            end
            
            ind_chop_on = logical(out(2,:));
            ind_chop_off = logical(out(1,:));
            
            %assign sorted data for array detector
            obj.sorted(:,:,1) = obj.sample(obj.ind_array(1,:),ind_chop_on);
            obj.sorted(:,:,2) = obj.sample(obj.ind_array(2,:),ind_chop_on);
